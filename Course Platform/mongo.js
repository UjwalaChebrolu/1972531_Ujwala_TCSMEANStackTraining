...
function insertToDatabase(course)
{
    mongoClient.connect(url, {useUnifiedTopology: true },(err1,client)=>{
        if(!err1){
            ...
            db.collection("Courses").insertOne(course,(err2,result)=>{
                    if(!err2){
   ...
   // console.log(courseId);
    mongoClient.connect(url, {useUnifiedTopology: true },(err1,client)=>{
        if(!err1){
            ...
            db.collection("Courses").deleteOne({"_id":courseId},(err2,result)=>{
                if(!err2){
...
function updateDatabase(courseId, newAmount)
{
    mongoClient.connect(url, {useUnifiedTopology: true },(err1,client)=>{
        if(!err1){
            ...
            db.collection("Courses").updateOne({"_id":courseId},{$set:{"amount":newAmount}},(err2,result)=>{
                if(!err2){
